...
// Глобальный инстанс состояния, чтобы не пересоздавать его при каждом Hot Reload
...
class AppState extends ChangeNotifier {
  ...
  // Добавляем отлов критических ошибок (например, если закрыты правила Firestore)
  String? lastError;
  ...
  void _initStreams() {
    // Используем плоскую структуру коллекции под каждую дату, чтобы исключить баги вложенного merge: true
    try {
      _db.collection('schedule_shifts_v2').snapshots().listen((snap) {
        ...
        for (var doc in snap.docs) {
          ...
          data.forEach((barista, typeIndex) {
            ...
            shifts[barista]![dateKey] = ShiftType.values[(typeIndex as num).toInt()];
          });
        }
        notifyListeners();
      }, onError: (e) {
      ...
      _db.collection('schedule_prefs_v2').snapshots().listen((snap) {
        ...
        for (var doc in snap.docs) {
          ...
          data.forEach((barista, typeIndex) {
            ...
            prefs[barista]![dateKey] = PrefType.values[(typeIndex as num).toInt()];
          });
        }
        notifyListeners();
      });
      _db.collection('logs_v2').orderBy('time', descending: true).limit(50).snapshots().listen((snap) {
...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  void _safeToggleShift(AppState state, String barista, DateTime date) {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Критическая ошибка: $e')));
    }
  }
  @override
  Widget build(BuildContext context) {
      ...
      children: [
        if (state.lastError != null)
          Container(
            ...
            child: Text(state.lastError!, style: TextStyle(color: Colors.red.shade900, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
          ),
